    ...
    <div className="mt-10">
        <div className="flex justify-between mb-5">
          ...
          <Link
            ...
            className={`text-gray-600 font-medium hover:text-gray-800 ${limit ? "block" : "hidden"}`}
          >
        ...
        <div className="flex flex-col gap-3">
          {(!limit ? recentTasks : recentTasks.slice(0, 3)).map((task) => (
            <div
              key={task.id}
              className="flex items-start justify-between gap-6 p-4 bg-white border border-gray-200 rounded-lg shadow-sm 
        hover:shadow-md transition-all duration-200 cursor-pointer border-l-4 border-l-gray-900"
            >
              {/* Left Side */}
